...
A matching Windows launcher ships in the repository, along with the flag file format if you would rather adapt your own.
...
Where **AdvancedStatus** is installed, the countdown also renders as a HUD bar. It is an optional dependency: without it the countdown runs and is announced in chat, and `hotwire check` reports which state you are in.
...
The problem space was mapped in part by reading **Smooth Restarter** by 2CHEVSKII. Hotwire shares no code with it.
