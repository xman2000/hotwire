...
def compressed(b, p):
    """ECMA-335 compressed unsigned int: 1, 2 or 4 bytes, told apart by the top bits.

    Written after the first Help extraction truncated every description over
    127 characters: a single length byte is only correct below 0x80, and the
    long ones are exactly the useful ones.
    """
    v = b[p]
    if v & 0x80 == 0:
        return v, p + 1
    if v & 0x40 == 0:
        return ((v & 0x3F) << 8) | b[p + 1], p + 2
    return ((v & 0x1F) << 24) | (b[p + 1] << 16) | (b[p + 2] << 8) | b[p + 3], p + 4


def named_args(blob):
    """The named arguments of a CustomAttribute, as {name: value}.

    Decodes properly rather than scanning, which `shows_in_admin_ui` does not:
    a scan cannot tell a Help string that happens to contain the word from a
    real flag, and it cannot read Help at all. Only the shapes Rust's convar
    attributes actually use are handled -- strings and bools -- and anything
    else stops the walk rather than guessing at an offset.
    """
    out = {}
    if not blob:
        return out
    try:
        blob = bytes(getattr(blob, "value", blob) or b"")
        if len(blob) < 4 or blob[0] != 1:
            return out
        p = 2
        count = struct.unpack_from("<H", blob, p)[0]
        p += 2
        for _ in range(count):
            p += 1                      # 0x53 field / 0x54 property
            etype = blob[p]
            p += 1
            if etype == 0x0E:           # string
                ln, p = compressed(blob, p)
                name = blob[p:p + ln].decode("utf8")
                p += ln
                if blob[p] == 0xFF:
                    out[name] = None
                    p += 1
                else:
                    vl, p = compressed(blob, p)
                    out[name] = blob[p:p + vl].decode("utf8", "replace")
                    p += vl
            elif etype == 0x02:         # bool
                ln, p = compressed(blob, p)
                name = blob[p:p + ln].decode("utf8")
                p += ln
                out[name] = bool(blob[p])
                p += 1
            else:
                break
    except Exception:
...
def emit_json(rows, build_note):
    ...
    something the plugin refuses, which is a worse hint than none.

    It is a HINT and never a validator: servers run builds we did not read, so
    a name missing from this file is not a name the panel may refuse.
    """
    catalogue = [
        {
            "name": r["convar"].lower(),
            "type": r["value_type"],
            "default": r["default"],
            "admin_ui": bool(r["admin"]),
            "help": r.get("help") or None,
        }
        for r in rows
        if "ServerVar" in r["kind"] or "ReplicatedVar" in r["kind"]
    ]
    catalogue.sort(key=lambda r: r["name"])
    print(json.dumps({
        "generated_from": build_note,
        "note": "Generated by tools/convars.py --json from a real Assembly-CSharp.dll. "
                "A hint for autocomplete, never a validator: a server may run a newer build than this.",
        "convars": catalogue,
...
def collect(path):
    ...
    def help_text(table, index):
        """Facepunch's own one-line description, when the attribute carries one."""
        for a, blob in ca_map.get((table, index), []):
            if a in CONVAR_ATTRS:
                said = named_args(blob).get("Help")
                if said:
...
if __name__ == "__main__":
    ...
    if "--json" in args:
        emit_json(rows, os.path.basename(args[0]))
    elif "--check" in args:
